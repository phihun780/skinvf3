...
- **Trình phối:** trong trang chỉ là bản xem trước (khung thấp ôm sát xe, xe phóng to 1.45×) (không bắt cử chỉ → vuốt vẫn cuộn trang). Chạm **Bắt đầu phối** → toàn màn hình (`.studio-embed.full`, khoá cuộn trang), kéo để xoay, **chụm 2 ngón để phóng to**, ✕ hoặc nút Back của điện thoại để thoát. Thanh dưới có thêm Hoàn tác / Làm lại. Bảng màu / decal là tấm trượt từ dưới lên; bảng decal thu gọn được (chọn decal xong tự thu gọn để chạm lên xe dán), camera tự đưa xe vào phần còn trống. Khung decal có tay cầm to cho ngón tay.
- **Dock điện thoại** (`src/ui/MobileDock.tsx`, thay bảng màu góc phải + bảng decal + thanh công cụ nổi): 1 tấm dính đáy màn hình, dưới cùng là hàng công cụ (Hoàn tác · Làm lại · Tự xoay · Reset · Lưu · Tải hình).
  - Màu sơn: dải tên vùng vuốt ngang (chạm tên = chọn vùng, vùng loé sáng; tên ngắn ở `SHORT_NAMES` trong `src/config/zones.ts`); chọn vùng rồi thì hiện dải ô màu 44px + ô màu tuỳ chọn, Bóng/Nhám.
  - Decal: chờ dán → chỉ 1 dòng "Chạm lên xe để dán" + Huỷ; đang chọn 1 decal → thanh chỉnh (Độ đậm, Lật, Chép sang bên kia, Xoá, Xong); còn lại → 3 tab Thư viện / Ảnh của bạn / Chữ + dải decal đã dán.
  - Dock tự báo mép trên (`useSheetTop`) → camera đặt xe giữa phần trống.
- **Lưu hình trên điện thoại:** cửa sổ toàn màn hình, nút lớn ở dưới. "Lưu hình" mở bảng chia sẻ của máy (Lưu vào Ảnh / Zalo / Messenger…), máy không hỗ trợ thì tải JPG; "Bản in (PNG)" chỉ tạo khi bấm (nặng).
...
- Sửa được **toàn bộ trang chủ**, mỗi mục một form (danh mục bên trái có chấm vàng = mục có thay đổi chưa lưu; xem trước tự cuộn tới mục đang sửa):
  ...
  - **Hero — decal + phụ kiện cho từng màu mockup** (`src/pages/cms/LookExtras.tsx`): bật/tắt phụ kiện ngay trong CMS; decal dán trực quan trong trình phối: "Mở trình phối với mẫu này" (ghi mẫu vào thiết kế đã lưu của trình duyệt, mở `/#phoi-xe` tab mới) → dán decal → quay lại bấm "Lấy thiết kế từ trình phối" (lấy màu + decal + phụ kiện; ảnh decal tự tải lên được đưa lên R2). Xe hero dựng decal bằng `ShowcaseDecals` (`src/three/Decals.tsx`) trên đúng các mesh của xe trưng bày.
...
3. Xuất Hình ảnh SkinVF3. ✅ (dọc A4 300dpi 2480×3508 PNG + bản nhẹ JPG, ghép bằng canvas 2D ở `src/poster/renderPoster.ts`; ảnh 3/4 trước + 4 góc chụp riêng nền trong suốt; ảnh 3/4 lớn + lưới 2×2 bốn góc; góc phải chỉ ghi ngày tạo; chân trang chỉ ghi skinvf3.pages.dev (mờ, nguồn mô hình 3D ghi trên web); không có bảng màu, mã thiết kế, decal; cửa sổ xem trước gắn vào <body> (nằm trên thanh trên, nền tối đặc); màu nền ở `src/config/poster.ts`)
