...
- **Phối màu trên điện thoại** (`src/ui/MobilePaint.tsx`, thay bảng màu góc phải): tấm gọn ở cuối màn hình — dải tên vùng vuốt ngang luôn có (chạm tên = chọn vùng, vùng loé sáng; tên ngắn ở `SHORT_NAMES` trong `src/config/zones.ts`), chọn vùng rồi thì hiện dải ô màu 44px vuốt ngang + ô màu tuỳ chọn, Bóng/Nhám, "Giống thân xe". Tấm bảng (màu / decal) tự báo mép trên (`useSheetTop`) → camera đặt xe giữa phần trống.
